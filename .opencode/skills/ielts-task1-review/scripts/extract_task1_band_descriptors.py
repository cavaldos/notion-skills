#!/usr/bin/env python3
"""Extract complete IELTS Academic Writing Task 1 band descriptors from a PDF."""
...
def extract(pdf_path: Path) -> dict[str, dict[str, str]]:
    ...
    with pdfplumber.open(str(pdf_path)) as pdf:
        for page in pdf.pages:
            ...
            if "Writing Task 1" not in text and "Task Achievement" not in text:
                continue
            for table in page.extract_tables():
                if not table or len(table) < 2:
                    continue
                headers = [clean_cell(cell) for cell in table[0]]
                if "Task Achievement" not in headers:
                    continue
                for row in table[1:]:
...
def main() -> None:
    ...
    if not descriptors:
        raise SystemExit("No Task 1 band descriptors were extracted")
    args.json.write_text(json.dumps(descriptors, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    write_markdown(descriptors, args.md)
    print(f"Extracted Task 1 descriptors for bands: {', '.join(descriptors.keys())}")
