#!/usr/bin/env python3
"""Download the "Express an opinion" Notion database to local JSON + Markdown.

Usage:
    python3 .opencode/skills/toeic-speaking-opinion/scripts/download_opinion_db.py [--outdir notion-data]

Reads NOTION_TOKEN from env var, .env (root) or .opencode/.env.
Saves (project root is auto-detected, so this works from any CWD):
    <outdir>/express_opinion/database.json   - database schema
    <outdir>/express_opinion/pages.json      - all page results (raw)
    <outdir>/express_opinion/data.md         - human-readable Markdown
"""
...
# slug -> (name, data_source_id)  — data source id from SKILL.md
DATABASES = {
    "express_opinion": (
        "Express an opinion",
        "3aaf312e-ab25-8020-8747-000ba003bf98",
    ),
}
